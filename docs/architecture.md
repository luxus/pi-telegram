...
- Telegram voice settings, provider selection, voice-menu setting projection, inline voice-setting action planning, transcription, speech preparation, synthesis, and Telegram voice-note delivery helpers live in `/lib/voice.ts`; Telegram voice inline-menu rendering and callback application live in `/lib/voice-menu.ts`; runtime port binding and prompt-turn enrichment live in `/lib/voice-runtime.ts`
