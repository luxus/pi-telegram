...
import type { TelegramConfig, TelegramVoiceConfig } from "./config.ts";
...
export const TELEGRAM_VOICE_MENU_VOICE_IDS = [
  "eve",
  "ara",
  "rex",
  "sal",
...
export const TELEGRAM_VOICE_MENU_LANGUAGES = ["de", "en", "auto"] as const;
export const TELEGRAM_VOICE_MENU_STYLES = [
  "literal",
  "rewrite-light",
  "rewrite-tags",
  "rewrite-strong",
] as const satisfies readonly TelegramSpeechStyle[];

export type TelegramVoiceMenuCommand = Exclude<
  TelegramVoiceCommand,
  { action: "status" } | { action: "provider" } | { action: "prompt" } | { action: "prompt-reset" }
>;

export type TelegramVoiceMenuActionPlan =
  | { handled: false }
  | { handled: true; command?: TelegramVoiceMenuCommand; message?: string };
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params) {
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: "Queued Telegram voice reply." }],
          details: {
            voiceId: params.voiceId || defaults.defaultVoiceId,
            language: params.language || defaults.defaultLanguage,
            alsoSendText,
...
export function resolveTelegramVoiceMenuSettings(
  settings: ResolvedTelegramVoiceSettings,
): TelegramVoiceMenuSettings {
  return {
...
export function getDefaultTelegramVoiceMenuSettings(): TelegramVoiceMenuSettings {
  return {
    enabled: false,
    provider: DEFAULT_TELEGRAM_VOICE_PROVIDER,
    replyWithVoiceOnIncomingVoice: false,
    autoTranscribeIncoming: false,
    alsoSendTextReply: true,
    voiceId: "eve",
    language: "auto",
    speechStyle: DEFAULT_TELEGRAM_SPEECH_STYLE,
  };
}

export function getTelegramVoiceMenuStatusSummary(
  settings: TelegramVoiceMenuSettings,
): string {
  return settings.enabled ? "on" : "off";
}

export function planTelegramVoiceMenuAction(
  action: string,
  value?: string,
): TelegramVoiceMenuActionPlan {
  if (action === "noop") return { handled: true };
  if (action === "back") return { handled: true };
  if (action === "toggle") {
    if (value !== "on" && value !== "off") {
      return { handled: true, message: "Invalid voice mode." };
    }
    return {
      handled: true,
      command: { action: "toggle", enabled: value === "on" },
      message: `Voice ${value}`,
    };
  }
  if (action === "reply") {
    if (value !== "on" && value !== "off") {
      return { handled: true, message: "Invalid reply mode." };
    }
    return {
      handled: true,
      command: { action: "reply", enabled: value === "on" },
      message: value === "on" ? "Reply with voice" : "Reply with text",
    };
  }
  if (action === "transcribe") {
    if (value !== "on" && value !== "off") {
      return { handled: true, message: "Invalid transcript mode." };
    }
    return {
      handled: true,
      command: { action: "transcribe", enabled: value === "on" },
      message: value === "on" ? "Transcribe on" : "Transcribe off",
    };
  }
  if (action === "text") {
    if (value !== "on" && value !== "off") {
      return { handled: true, message: "Invalid text-copy mode." };
    }
    return {
      handled: true,
      command: { action: "text", enabled: value === "on" },
      message: value === "on" ? "Text copy on" : "Voice only",
    };
  }
  if (action === "style" && isTelegramSpeechStyle(value)) {
    return {
      handled: true,
      command: { action: "style", style: value },
      message: `Style: ${value}`,
    };
  }
  if (action === "voice" && value) {
    return {
      handled: true,
      command: { action: "voice", voiceId: value },
      message: `Voice: ${value}`,
    };
  }
  if (action === "lang" && value) {
    return {
      handled: true,
      command: { action: "language", language: value },
      message: `Language: ${value}`,
    };
  }
  return { handled: true, message: "Unknown voice action." };
}

function isTelegramSpeechStyle(value: string | undefined): value is TelegramSpeechStyle {
  return TELEGRAM_VOICE_MENU_STYLES.includes(value as TelegramSpeechStyle);
}
